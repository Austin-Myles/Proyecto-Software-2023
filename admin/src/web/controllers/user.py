...
from src.web.utils.forms.auth import ConfirmGoogleForm, ConfirmRegisterForm
...
def confirm_page(user_id):
    ...
    """
    return render_template(
        "users/user-confirm.html", user_id=user_id, form=ConfirmRegisterForm()
    )
...
def confirm_user(user_id):
    ...
    try:
        if not form.validate():
            raise exceptions.BadRequest("Parametros invalidos")
...
@user_routing.get("/confirm-google/<user_id>")
def confirm_google_page(user_id):
    """
    Returns the confirmation page
    :param user_id: user id
    """
    return render_template(
        "users/user-confirm-google.html", user_id=user_id, form=ConfirmGoogleForm()
    )


@user_routing.post("/confirm-google/<user_id>")
def confirm_google_user(user_id):
    """
    Confirms a user registration (complete registration).
    If correct, creates a new user and deletes the temp user,
    then redirects with success to login.
    :param user_id: user id
    """
    form = ConfirmGoogleForm(request.form)
    simpleuser = simple_user_dbo.get_by_id(user_id)
    try:
        if not form.validate():
            raise exceptions.BadRequest("Parametros invalidos")
        ...
        flash(error.description, "error")
        return redirect(url_for("users.confirm_google_page", user_id=user_id))
    return redirect(url_for("auth.login_page"))
