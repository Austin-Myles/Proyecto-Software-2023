...
def validate_phone_number(form, field):
    ...
    if not re.match(r'^\d+$', field.data):
        raise ValidationError("Debe ingresar un número de teléfono válido (solo números).")

class ConfigurationUpdateForm(FlaskForm):
    elements_per_page = StringField('Elementos por página', validators=[
        DataRequired("Campo requerido"),
        validate_elements_per_page
    ])
    phone_number = StringField('Número de teléfono de contacto', validators=[
        DataRequired("Campo requerido"),
        Length(min=1, message="Debe ingresar un número de teléfono."),
        validate_phone_number
    ])
    email = StringField('Correo electrónico de contacto', validators=[
        DataRequired("Campo requerido"),
