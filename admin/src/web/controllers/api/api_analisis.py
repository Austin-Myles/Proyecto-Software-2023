...
from src.schemas.service_schema import service_analisis_schema
...
def services_types():
    ...
    services_count = {
        ServiceTypes.ANALYSIS.value: 0,
        ServiceTypes.CONSULTING.value: 0,
        ServiceTypes.DEVELOPMENT.value: 0,
    }
    print(services_count)
    data = service_dbo.requests_by_type()
    for service in data:
        services_count[service[0].value] = service[1]
    return make_response(jsonify({"data": services_count}), 200)
