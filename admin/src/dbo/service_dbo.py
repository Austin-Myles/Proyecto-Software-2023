...
def full_text_search(q, service_type, page=1, per_page=1):
    """
    Returns all services by the search param
    :param q: searchbar input
    :param service_type: service type
    :param page: page number
    :param per_page: page size
    """
    query = db.session.query(ServiceModel)
    if q != "":
        qterm = " & ".join(word + ":*" for word in q.split())
        tsquery = sa.func.to_tsquery("argentino", qterm)
        query = query.filter(ServiceModel.ts_vector.op("@@")(tsquery))
        query = query.order_by(sa.func.ts_rank(ServiceModel.ts_vector, tsquery).desc())
